...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals,
                                                            ...
                                                            LocalTime endTime,
                                                            int caloriesPerDay) {
        Map<LocalDate, Integer> datesWithExcess = new HashMap<>();

        for (UserMeal meal : meals) {
            datesWithExcess.merge(meal.getDateTime().toLocalDate(), meal.getCalories(), Integer::sum);
        }

        List<UserMealWithExcess> resultList = new ArrayList<>();

        for (UserMeal meal : meals) {
            boolean isExcess = datesWithExcess.get(meal.getDateTime().toLocalDate()) > caloriesPerDay;
            ...
            if (isBetweenInclusive(meal.getDateTime().toLocalTime(), startTime, endTime)) {
                resultList.add(new UserMealWithExcess(meal.getDateTime(),
                        meal.getDescription(), meal.getCalories(), isExcess));
            }
        }
        return resultList;
    }
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals,
                                                             ...
                                                             LocalTime endTime,
                                                             int caloriesPerDay) {
        Map<LocalDate, Integer> datesWithExcess = meals.stream()
        ...
        return meals.stream()
                ...
                .map(meal -> new UserMealWithExcess(meal.getDateTime(), meal.getDescription(), meal.getCalories(),
                        datesWithExcess.get(meal.getDateTime().toLocalDate()) > caloriesPerDay))
